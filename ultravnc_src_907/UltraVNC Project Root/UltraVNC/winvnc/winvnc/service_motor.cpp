...
char service_name[256]="SihlSuporte";
char *app_name = "SihlSuporte";
...
void set_service_description()
{
	...
	_snprintf(tempName, sizeof tempName, "SYSTEM\\CurrentControlSet\\Services\\%s", "SihlSuporte");
...
int install_service(void) {
   ...
   /* service=CreateService(scm,service_name, service_name, SERVICE_ALL_ACCESS,
	...
	service = CreateService(scm, "SihlSuporte", "SihlSuporte", SERVICE_ALL_ACCESS,
		SERVICE_WIN32_OWN_PROCESS,
		SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, service_path,
		NULL, NULL, VNCDEPENDENCIES, NULL, NULL);
    if(!service) {
